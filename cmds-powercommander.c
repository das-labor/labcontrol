...
typedef struct name_value_t {
	uint8_t value;
	char *name;
	char *desc;
} NameValue;

NameValue nametovalue[] = {
...
void cmd_powercommander_help() {
	int i = 0;
	puts (	"Das ganze setzt sich wie folgt zusammen:\n\n"
		"lapcontrol powercommander <class> <object> <function> <value>\n\n"
		"im folgendem sind alle Klassen mit (C) , alle Objekte mit (O) und alle Funktionen mit (F) markiert\n\n"
		"es findet keine logische ueberpruefung statt... d.h. man kann das auch falsch zusammenstellen, also bitte die eigene logik ueberpruefen.\n"
		"Es macht beispielsweise keinen Sinn die Klasse PWMs auszuwaehlen und dann ein Switchobjekt zu definiern und dieses am ende setzen zu wollen auf einen wert von 0xff oder so.\n\n"
		"Was aber geht ist in etwa folgendes: alle switche haben ein, aus, status; alle pwms haben set, get; virtuelle Objekte sind eigenstaendig definiert\n");
